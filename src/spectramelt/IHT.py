...
def CIHT(A, y, k, num_iterations=1000, learning_rate=0.001, error_thresh=0.001):
    """
    Iterative Hard Thresholding (IHT) for complex-valued sparse recovery.

    Args:
        A (np.ndarray): Measurement matrix (can be complex-valued).
        y (np.ndarray): Observed measurements (can be complex-valued).
        k (int): Desired sparsity level (number of non-zero elements).
        num_iterations (int): Number of iterations for the algorithm.
        learning_rate (float): Step size for the gradient descent update.

    Returns:
        np.ndarray: Estimated sparse signal (complex-valued).
    """
    n = A.shape[1]  # Number of columns in A (signal dimension)
    x = np.zeros(n, dtype=np.complex128)  # Initialize estimated signal with complex zeros
    residual = y - A @ x  # Initial residual (error)

	# for _ in range(num_iterations):
    while num_iterations > 0 and np.linalg.norm(residual) > error_thresh:
        num_iterations -= 1
		# 1. Gradient step
        gradient = A.conj().T @ residual  # Compute the gradient (using conjugate transpose for complex data)
        x = x + learning_rate * gradient  # Update the estimated signal

		# 2. Hard thresholding
		# Get the indices of the k largest magnitudes
        indices = np.argsort(np.abs(x))[-k:]

		# Create a new vector with only the k largest magnitude elements
        x_thresholded = np.zeros(n, dtype=np.complex128)
        x_thresholded[indices] = x[indices]

        x = x_thresholded # Update x with the thresholded version
		# 3. Update residual
        residual = y - A @ x  # Update the residual
...
# If the current vector is zero, we take the largest element in d
			...
			max_mu = np.max(mu,initial=max_mu)
			min_mu = np.min(mu,initial=min_mu)
			mu = min_mu
			s = s_old + mu*d
...
# Are we done yet?
			...
			if (not done):
				...
				oldERR = ERR
			if (verbose):
				print("Iter={0} - mu={1:.4f} - gap={2:.4e} - target={3:.4e}".format(Count, mu, gap, thresh))
	return s, err_mse