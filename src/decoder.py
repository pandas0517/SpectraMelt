...
def create_test_set(dictionary, system_params, wave_params, filter_params):
    ...
    for test_freq_list in test_freq_tot_list:
        for test_freq in test_freq_list:
            test_wave_params = []
            test_wave_param = {'amp': 0,
                         'freq': 0,
                         'phase': 0}
            for freq in test_freq:
                test_wave_param['amp'] = 1
                test_wave_param['freq'] = freq
                test_wave_params.append(test_wave_param)
            x, t, num_tones = multi_tone_sine_wave(system_params, test_wave_params, filter_params)
            xf = fft(x)
...
def create_decoder(dictionary, test_set):
