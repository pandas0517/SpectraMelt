...
from utility import replace_file
...
class NYFR_Test_Harness:
    ...
    def create_dfs(self, nyfr=None, filenames=None, directories=None):
        ...
        # df = pd.read_pickle(recovery_df_file_path)
        # recovery_df_file_path_csv = replace_extension(recovery_df_file_path, "csv")
        # Save the DataFrame to a CSV file
        # df.to_csv(recovery_df_file_path_csv, index=False)
        ...
        if os.path.exists(recovery_df_file_path):
            recovery_df = pd.read_pickle(recovery_df_file_path)
        else:
            recovery_df = pd.DataFrame({'file_name': pd.Series(dtype='str'),
                        'num_tones': pd.Series(dtype='str'),
                        'noise_level': pd.Series(dtype='str'),
                        'phase_shift': pd.Series(dtype='str'),
                        'f_mod': pd.Series(dtype='float'),
                        'f_delta': pd.Series(dtype='float'),
                        'dictionary_type': pd.Series(dtype='str'),
                        'recovery_method': pd.Series(dtype='str')})
        for mode in recovery_params['modes']:
            recovery_base_path = self.recovery_dir[dictionary_params['version']][recovery_params['type']]
            if ( recovery_params['type'] == 'MLP1' ):
                recovery_base_path = os.path.join(recovery_base_path,
                    ...
                    for input_tones, _ in self.input_set_params["input_tones"]:
                        ...
                        for f_mod, f_mod_value in self.input_set_params["f_mods"]:
                            for f_delta, f_delta_value in self.input_set_params["f_deltas"]:
                                ...
                                recovery_df.loc[len(recovery_df), recovery_df.columns[:8].tolist()] = [recovery_file_path,
                                                                                                    input_tones,
                                                                                                    noise_level,
                                                                                                    phase_shift,
                                                                                                    f_mod_value,
                                                                                                    f_delta_value,
                                                                                                    dictionary_params['type'],
                                                                                                    recovery_params['type']]
    ...
    def __add_columns_recovery_df(self, recovery_df, recovery_sig_set_size, recovery_df_path):
        for stats in range(recovery_sig_set_size):
            num_rec_freq = "num_rec_freq_" + str(stats)
            recovery_df[num_rec_freq] = 0.0
            num_spur_freq = "num_spur_freq_" + str(stats)
            recovery_df[num_spur_freq] = 0.0
            ave_rec_mag_err = "ave_rec_mag_err_" + str(stats)
            recovery_df[ave_rec_mag_err] = 0.0
            total_input_tones = "total_input_tones_" + str(stats)
            recovery_df[total_input_tones] = 0.0
            rec_tone_thresh = "rec_tone_thresh_" + str(stats)
            recovery_df[rec_tone_thresh] = 0.0
    ...
    def sample_signals(self, data=None, update_sampled_time=False, sample_rate=None, points_per_second=None, t=None):
        if data is None:
            data = self.nyfr.get_time()
        if sample_rate is None:
            sample_rate = self.nyfr.get_adc_clock_freq()
        if points_per_second is None:
            points_per_second = self.nyfr.get_points_per_second()
        if t is None:
            t = self.nyfr.get_time()
        return self.nyfr.sample_signals(data=data, update_sampled_time=update_sampled_time, sample_rate=sample_rate, points_per_second=points_per_second, t=t)
    ...
    def get_points_per_second(self):
        return self.nyfr.get_points_per_second()

    def get_time(self):
        return self.nyfr.get_time()
